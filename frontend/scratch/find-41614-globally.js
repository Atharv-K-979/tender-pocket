...
async function checkChunk(chunk) {
  ...
  try {
    ...
    // Check if the chunk contains module 41614 definition
    // Usually it looks like: 41614:function(e,t,s) or "41614":function(e,t,s) or similar
    const term1 = '41614:';
    const term2 = '"41614":';
    
    const index1 = res.data.indexOf(term1);
    const index2 = res.data.indexOf(term2);
    
    if (index1 !== -1 || index2 !== -1) {
      console.log(`\n*** Chunk ${chunk} contains match for module 41614! ***`);
      const index = index1 !== -1 ? index1 : index2;
      console.log(res.data.slice(index, index + 2500));
    }
  } catch (err) {
    // ignore
  }
...
async function run() {
  console.log("Searching chunks for module 41614...");
