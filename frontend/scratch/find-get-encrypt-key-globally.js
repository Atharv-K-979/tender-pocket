...
async function checkChunk(chunk) {
  ...
  try {
    ...
    // Search for getEncryptkey or encrypt-data references
    const index = res.data.indexOf('getEncryptkey');
    if (index !== -1) {
      console.log(`\n*** Chunk ${chunk} contains getEncryptkey! ***`);
      // Print first 5 matches of getEncryptkey context
      let idx = 0;
      while ((idx = res.data.indexOf('getEncryptkey', idx)) !== -1) {
        console.log(`  Reference at ${idx}:`);
...
async function run() {
  console.log("Searching chunks for getEncryptkey...");
